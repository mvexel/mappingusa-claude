...
def check_first_edit(changeset_id):
    ...
    try:
        ...
        user_changesets_url = f"https://api.openstreetmap.org/api/0.6/changesets?user={uid}"
...
def summarize_changeset():
    try:
        ...
        if is_first_edit is None:
            ...
            if error:
                return jsonify({
                    "error": "Failed to verify edit status",
                    "details": error
                }), 400
...
def health_check():
    ...
    # Check OSM API
    try:
        osm_response = requests.get("https://api.openstreetmap.org/api/0.6/capabilities")
